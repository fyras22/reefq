...
export async function GET(request: Request) {
  try {
    ...
    console.log(`- Hash exists: ${!!hash && hash.length > 0 ? "Yes" : "No"}`);
    ...
    if (error) {
      console.error(
        `[Auth Callback] Error from OAuth provider: ${error}: ${errorDescription}`
      );
    ...
    if (hash && hash.includes("access_token")) {
      console.log(
        "[Auth Callback] Hash contains access_token, forwarding to set-login-flag"
      );

      // Forward the hash to the client-side handler
      return NextResponse.redirect(
        new URL(
          `/auth/set-login-flag?redirectTo=${encodeURIComponent(next)}&hash=${encodeURIComponent(hash)}`,
          origin
        )
      );
    }
    ...
    if (code) {
      ...
      try {
        ...
        if (!data.session) {
          console.error(
            "[Auth Callback] No session returned after code exchange"
          );
          return NextResponse.redirect(
        ...
        // Redirect to the set-login-flag page which will handle session setup
        return NextResponse.redirect(
          new URL(
            `/auth/set-login-flag?redirectTo=${encodeURIComponent(next)}`,
            origin
          )
        );
    ...
    return NextResponse.redirect(
      `${origin}/auth/login?error=${encodeURIComponent("Missing authentication parameters")}`
    );
  } catch (error: any) {
    ...
    return NextResponse.redirect(
      `${fallbackOrigin}/auth/login?error=${encodeURIComponent("Something went wrong during authentication")}`
    );
  }
}
