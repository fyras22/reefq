  ...
  return (
    <section className="py-24 bg-gray-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          {metrics.map((metric, index) => (
            <div key={metric.id} className="bg-white rounded-lg shadow-md p-6">
              ...
              <p className="mt-4 text-3xl font-bold text-nile-teal">{metric.value}</p>
        ...
        >
          <h3 className="text-xl font-semibold text-gray-900 mb-4">{t('performance.chart.title')}</h3>
          <div className="h-64 bg-gray-100 rounded-lg flex items-center justify-center">
            <p className="text-gray-500">{t('performance.chart.placeholder')}</p>
          </div>
          <p className="mt-4 text-sm text-gray-600">{t('performance.chart.description')}</p>
        </motion.div>
      </div>
    </section>
  );
