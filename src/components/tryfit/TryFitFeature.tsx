...
type TryFitStep = 'jewelry-selection' | 'method-selection' | 'try-on' | 'results';

export default function TryFitFeature({ t, isRTL = false }: TryFitFeatureProps) {
  ...
  const [currentStep, setCurrentStep] = useState<TryFitStep>('jewelry-selection');
          ...
          <PageTransition type="fade">
            <div className="space-y-6">
              <div className="text-center">
                <h2 className="text-2xl font-bold text-gray-900">{t('tryFit.selectJewelry')}</h2>
                <p className="mt-2 text-gray-600">{t('tryFit.selectJewelryDesc')}</p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {SAMPLE_JEWELRY.map((jewelry) => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      <h3 className="font-semibold text-gray-900">{jewelry.name}</h3>
                      <p className="text-nile-teal font-medium">${jewelry.price.toLocaleString()}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
      ...
      case 'method-selection':
        return (
          <PageTransition type="fade">
            <div className="space-y-6">
              <div className="text-center">
                <h2 className="text-2xl font-bold text-gray-900">{t('tryFit.selectMethod')}</h2>
                <p className="mt-2 text-gray-600">{t('tryFit.selectMethodDesc')}</p>
              </div>
              ...
              {selectedJewelry && (
                <div className="bg-gray-50 rounded-xl p-4 flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="font-medium text-gray-900">{selectedJewelry.name}</h3>
                    <p className="text-sm text-gray-500">{selectedJewelry.description}</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                >
                  ...
                  <h3 className="font-semibold text-gray-900 mb-2">{t('tryFit.cameraMethod')}</h3>
                  <p className="text-sm text-gray-600">{t('tryFit.cameraMethodDesc')}</p>
                </motion.div>
                ...
                >
                  ...
                  <h3 className="font-semibold text-gray-900 mb-2">{t('tryFit.handMethod')}</h3>
                  <p className="text-sm text-gray-600">{t('tryFit.handMethodDesc')}</p>
                </motion.div>
                ...
                >
                  ...
                  <h3 className="font-semibold text-gray-900 mb-2">{t('tryFit.sizeGuideMethod')}</h3>
                  <p className="text-sm text-gray-600">{t('tryFit.sizeGuideMethodDesc')}</p>
                </motion.div>
              </div>
            </div>
          </PageTransition>
                ...
                <div className="space-y-6">
                  <div className="relative w-full bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="mt-4 p-4 bg-white rounded-lg border border-gray-200">
                      <h3 className="text-lg font-medium text-gray-900">{t('tryFit.followSteps')}</h3>
                  ...
                  >
                    {isLoadingModel ? t('tryFit.processing') : t('tryFit.completeHandSizing')}
                  </Button>
                </div>
              )}
              ...
              {tryOnMethod === 'sizing-guide' && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between bg-gray-50 p-3 rounded-lg">
                    <span className="text-gray-700">{t('tryFit.sizeFormat')}</span>
                    <button
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
                    {RING_SIZES.map((size) => (
                      ...
                      >
                        <span className="block text-lg font-medium">{getSizeDisplayValue(size)}</span>
                        <span className="text-xs text-gray-500">{size.sizeDiameter}mm</span>
                      </motion.button>
                    ))}
                  </div>
                  ...
                  <div className="bg-blue-50 rounded-lg p-4 flex items-start space-x-3">
                    ...
                    <div>
                      <h4 className="font-medium text-blue-700">{t('tryFit.sizeInfo')}</h4>
                      <p className="mt-1 text-sm text-blue-600">{t('tryFit.sizeInfoDesc')}</p>
                    </div>
                  </div>
                </div>
              )}
      ...
      case 'results':
        return (
          <PageTransition type="fade">
            <div className="space-y-6">
              <div className="text-center">
                <h2 className="text-2xl font-bold text-gray-900">{t('tryFit.results')}</h2>
                ...
                <div className="space-y-6">
                  <div className="bg-gradient-to-br from-nile-teal/10 to-pharaonic-gold/10 rounded-xl p-6">
                    <div className="text-center space-y-2">
                      <p className="text-gray-600">{t('tryFit.recommendedSize')}</p>
                      <div className="flex justify-center items-center space-x-4">
                        ...
                        <div className="bg-white px-3 py-1 rounded-full text-xs font-medium">
                          {sizeUnit === 'US' ? `${selectedSize.sizeEU} EU` : `${selectedSize.sizeUS} US`}
                        </div>
                      </div>
                      <p className="text-sm text-gray-500">{selectedSize.sizeDiameter}mm {t('tryFit.diameter')}</p>
                    </div>
                  ...
                  <div className="bg-white rounded-xl shadow-md overflow-hidden">
                    ...
                    <div className="p-4 flex items-center space-x-4">
                      ...
                      <div>
                        <p className="text-sm text-gray-500">{selectedJewelry.description}</p>
                        <p className="text-nile-teal font-medium mt-1">${selectedJewelry.price.toLocaleString()}</p>
                      </div>
                    </div>
                  </div>
    ...
    <div className="max-w-lg mx-auto p-4 sm:p-6">
        ...
        {isLoadingModel && (
          ...
          >
            <div className="bg-white p-6 rounded-xl max-w-sm w-full text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-900">{t('tryFit.processing')}</h3>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          {['jewelry-selection', 'method-selection', 'try-on', 'results'].map((step, index) => (
              ...
              <motion.div
                className={`w-8 h-8 rounded-full flex items-center justify-center ${
                  currentStep === step
                    ...
                    : index < ['jewelry-selection', 'method-selection', 'try-on', 'results'].indexOf(currentStep)
